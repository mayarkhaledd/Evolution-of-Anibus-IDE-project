...
class Widget(QWidget):
    ...
    def initUI(self):
        ...
        labelForArgs.setText("Enter function Parameter here ended by a semi colon ';' ") #as the function accepts one parameter to print
...
class UI(QMainWindow):
    ...
    #improvmnets are done to the run function, to accept functions with parameters
    def run(self):
        mytext = text.toPlainText() #taking the text written in the code section
        text2.clear()
        if len(mytext) > 7 and mytext[0:4] == 'def ':  #cheching if the written text starts with def, so it is a function
            name = mytext[0:mytext.find("(")]
            call = name + "("
            parameters = paramsLine.text()

            for i in parameters: #handeling parameters of the function
                call += i
            ...
            try:
                ...
                sys.stdout = result
                exec(mytext + "\n" + call)
                text2.append(result.getvalue()) #print in console
                sys.stdout = original_stdout
            except:
                text2.append("Exception is found")
        else:  #if the written code is not a function it will be executed normally
            try:
                codeOut = StringIO()
                sys.stdout = codeOut
                exec(mytext)
                text2.append(codeOut.getvalue()) # Show result in console
